...
function AppProvider({ children }) {
    ...
    const [isVisibleReactionsModal, setIsVisibleReactionsModal] =
        useState(false);
    ...
    const appConfig = JSON.parse(
        localStorage.getItem("dtuan_space_config")
    ) ?? { appTheme: "light" };

    const setAppConfig = (key, value) => {
        appConfig[key] = value;
        localStorage.setItem("dtuan_space_config", JSON.stringify(appConfig));
    };
    ...
    useEffect(() => {
        const updateUserActive = () => {
            ...
            const usersActiveTime = users.map((user) => {
                ...
                const isActive =
                    Math.floor((currentTime - activeTime) / 60) <= 4;
        ...
        if (rooms.length > 0 && usersActiveStatus.length > 0) {
            const roomsActive = rooms.map((room) => {
                ...
                if (
                    membersActive.some(
                        (member) =>
                            member.isActive === true && member.uid !== uid
                    )
                ) {
                    return {
                        roomId: room.id,
    ...
    const joinGlobalChat = (user) => {
        ...
        const roomid = "NJaugoP27a7kpQ8gYPlH";
        ...
        if (room) {
            if (room.role.includes("person")) {
                if (room.members.length === 1) {
                    ...
                    // Return friend name
                    const friendUID = room.members.find(
                        (members) => members !== uid
                    );
